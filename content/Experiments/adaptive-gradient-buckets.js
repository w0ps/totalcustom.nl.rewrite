...
function getBuckets( gradient, scalars, diffThreshold, max, bucketDecorator ) {
        ...
        if( index ) {
          ...
          if( difference ) {
            memo.transitions.push( {
              ...
              length: point2.t - memo.startPoint.t,
              diff: difference,
              biasedLength: ( point2.t - memo.startPoint.t ) * difference,
              steps: 1
            } );
  ...
  do {
    ...
    if (
      ...
      if( !pointsByDiff.length ) {
        ...
        relativeD1 = ( point1.t || !point2.t ) ? point1.t / ( point1.t + ( 1 - point2.t ) ) : 0.5;
        ...
        if( maxBuckets === 2 ) {
          ...
          return [ {
            ...
            t: ( workingGradient[ 0 ].t + workingGradient[ 1 ] ) / 2,
            values: scalars.reduce( function getValues( memo, scalar ) {
      ...
      transition.length = point2.t - point1.t;
  ...
  var lastPosition = 0,
      ...
      buckets = transitions.reduce( function createBuckets( buckets, transition, index, list ) {
        ...
        while( step < steps ) {
          ...
          buckets.push( createBucket( lastPosition, edge, t ) );
        ...
        if ( index + 1 === transitionsAmt ) {
          buckets.push( createBucket( lastPosition, 1, 1 ) );
        }

        return buckets;

        function createBucket( start, end, t ) {
          var bucket = {
                start: start,
                end: end,
                t: t,
